...
async function createDeveloperDatabase(developerUsername) {
    ...
    try {
        ...
        // Create studentdetails table with dateofbirth
        await connection.query(`
...
app.post('/register-student', authenticate, authorize(['developer', 'admin', 'subadmin', 'teacher']), async (req, res) => {
    const { username, mobile, password, confirmPassword, studentClass, name, fatherName, motherName, address, phone1, dateofbirth } = req.body;
    ...
    try {
        ...
        await connection.query(
            'INSERT INTO studentdetails (username, name, father_name, mother_name, class, address, phone1, student_id, dateofbirth, created_by, developer) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [username, name || username, fatherName || '', motherName || '', studentClass, address || '', phone1 || mobile, studentResult.insertId.toString(), dateofbirth || null, req.username, developerUsername]
        );
...
app.post('/api/students', authenticate, authorize(['developer', 'admin', 'subadmin', 'teacher']), upload.single('image'), async (req, res) => {
    try {
        ...
        const [result] = await connection.query(
            'INSERT INTO studentdetails (username, name, father_name, mother_name, class, address, phone1, student_id, image_id, dateofbirth, created_by, developer) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [studentData.username, studentData.name, studentData.fatherName, studentData.motherName, studentData.class, studentData.address, studentData.phone1, studentRecord.insertId.toString(), imageId, studentData.dateofbirth || null, req.username, req.developer || req.username]
        );
...
app.patch('/api/students/:id', authenticate, authorize(['developer', 'admin', 'subadmin', 'teacher', 'student']), async (req, res) => {
    try {
        ...
        let allowedFields;
        if (req.role === 'student') {
            allowedFields = ['name', 'father_name', 'mother_name', 'address', 'phone1', 'class', 'dateofbirth'];
        } else {
...
app.patch('/api/students/:id/personal-details', authenticate, authorize(['developer', 'admin', 'subadmin', 'teacher', 'student']), async (req, res) => {
    try {
        ...
        const allowedFields = ['name', 'fatherName', 'motherName', 'address', 'phone1', 'class', 'dateofbirth'];
        const updateFields = {};
        const fieldMapping = {
            'name': 'name',
            'fatherName': 'father_name',
...
app.patch('/api/student/profile', authenticate, authorize(['student']), async (req, res) => {
    try {
        ...
        const allowedFields = ['name', 'fatherName', 'motherName', 'address', 'phone1', 'dateofbirth'];
        const updateFields = {};
        const fieldMapping = {
            'name': 'name',
            'fatherName': 'father_name',
        ...
        for (const student of students) {
            ...
            try {
                ...
                await connection.query(
                    ...
                    (username, name, father_name, mother_name, class, address, phone1, student_id, admissionnumber, dateofbirth, created_by, developer) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                    [
                        mappedStudent.username, 
                        mappedStudent.name, 
