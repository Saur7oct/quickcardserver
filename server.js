...
async function migrateDatabase(connection, dbName) {
    try {
        ...
        const [dobColumns] = await connection.query(`
            SELECT COLUMN_NAME 
        ...
        if (houseColumns.length === 0) {
            ...
            await connection.query(`
                ALTER TABLE studentdetails 
                ADD COLUMN student_house VARCHAR(255) NULL
            `);
...
app.post('/api/students', authenticate, authorize(['developer', 'admin', 'subadmin', 'teacher']), upload.single('image'), async (req, res) => {
    try {
        ...
        const [result] = await connection.query(
            ...
            [studentData.username, studentData.name, studentData.fatherName, studentData.motherName, studentData.studentHouse || '', studentData.class, studentData.address, studentData.phone1, studentRecord.insertId.toString(), imageId, studentData.dateofbirth || null, req.username, req.developer || req.username]
        );
...
app.patch('/api/students/:id', authenticate, authorize(['developer', 'admin', 'subadmin', 'teacher', 'student']), async (req, res) => {
    try {
        ...
        Object.keys(updateData).forEach(field => {
            if (allowedFields.includes(field) || allowedFields.includes(fieldMapping[field])) {
...
app.post('/api/students/batch', authenticate, authorize(['developer', 'admin', 'subadmin']), async (req, res) => {
    try {
        ...
        for (const student of students) {
            ...
            if (mappedStudent.username && !mappedStudent.username.startsWith('student_')) {
